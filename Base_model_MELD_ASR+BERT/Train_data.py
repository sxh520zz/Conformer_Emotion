...
with open('../MELD_Fea/train_data_map_for-Mulit-speaker.pickle', 'rb') as file:
    train_data= pickle.load(file)
with open('../MELD_Fea/test_data_map_for-Mulit-speaker.pickle', 'rb') as file:
    dev_data = pickle.load(file)
with open('../MELD_Fea/dev_data_map_for-Mulit-speaker.pickle', 'rb') as file:
...
def Bert(x):
    b = x.lower()
    print(x)
    #line = gensim.utils.simple_preprocess(str(x))
    #b = ' '.join(line)
    if b:
        x = b
        input_data = []
        input_data.append(x)
        return bc.encode(input_data)
    else:
        del x
        return [[],[]]
