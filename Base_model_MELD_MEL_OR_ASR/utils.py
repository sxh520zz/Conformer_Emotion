...
def Feature(data,args):
    ...
    for i in range(len(data)):
        input_data_spec.append(data[i]['trad_data_1'][0])
    a = [0.0 for i in range(args.utt_insize_1)]
    a = np.array(a)
    input_data_spec_CNN = []
    ...
    input_data_spec_2 = []
    for i in range(len(data)):
        input_data_spec_2.append(data[i]['trad_data_2'])
    a = [0.0 for i in range(args.utt_insize_2)]
    a = np.array(a)
    input_data_spec_CNN_2 = []
    for i in range(len(input_data_spec_2)):
        ha = []
        if(len(input_data_spec_2[i]) < 300):
            for z in range(len(input_data_spec_2[i])):
                ha.append(np.array(input_data_spec_2[i][z]))
            len_zero = 300 - len(input_data_spec_2[i])
            for x in range(len_zero):
                ha.append(np.array(a))
        if(len(input_data_spec_2[i]) >= 300):
            for z in range(len(input_data_spec_2[i])):
                if(z < 300):
                    ha.append(np.array(input_data_spec_2[i][z]))
        ha = np.array(ha)
        input_data_spec_CNN_2.append(ha)
    input_data_spec_CNN_2 = STD(input_data_spec_CNN_2)
    ...
    input_label_org = []
    for i in range(len(data)):
        input_label_org.append(data[i]['label_emotion'])
    return input_data_spec_CNN,input_data_spec_CNN_2,input_label,input_data_id,input_label_org
...
def Get_data(data,train_data,test_data,args):
    ...
    train_dataset = subDataset(input_train_data_spec,input_train_data_spec_CNN,label)
    test_dataset = subDataset(input_test_data_spec,input_test_data_spec_CNN,label_test)
